...
const { getConversationById, getMessagesByConversationId } = conversationsStore
...
<template>
  <div class="group-chat-window">
    <div class="group-chat-window__header">
      <div class="group-chat-window__title">{{ conversation?.title ?? '群聊' }}</div>
    </div>

    <div ref="messagesContainer" class="group-chat-window__messages">
    ...
    <div class="group-chat-window__input-area">
      <div class="group-chat-window__toolbar">
        <div style="position: relative">
          <button class="chat-toolbar-btn" type="button" title="表情" @click="showEmojiPicker = !showEmojiPicker">表</button>
        ...
        <button class="chat-toolbar-btn" type="button" title="图片" @click="handleImageUpload">图</button>
        <button class="chat-toolbar-btn" type="button" title="文件" @click="handleFileUpload">件</button>
      </div>
      <div class="group-chat-window__input-wrapper">
        <textarea
          v-model="inputText"
          class="group-chat-window__input"
          placeholder="输入群消息"
          @keydown.enter.exact.prevent="handleSend"
        ></textarea>
      </div>
      <div class="group-chat-window__actions">
        <button class="chat-send-btn" type="button" @click="handleSend">发送到群</button>
      </div>
    </div>
  </div>
</template>
...
.group-chat-window__header {
  padding: 12px 16px;
  border-bottom: 1px solid rgba(74, 131, 186, 0.14);
  background: rgba(255, 255, 255, 0.62);
}

.group-chat-window__title {
  font-size: 14px;
  font-weight: 700;
  color: #2d5f8f;
}

.group-chat-window__subtitle {
  margin-top: 4px;
  font-size: 11px;
  color: rgba(45, 95, 143, 0.58);
}

.group-chat-window__messages {
  flex: 1;
  overflow-y: auto;
  padding: 16px;
  display: flex;
...
.group-chat-window__input-area {
  border-top: 1px solid rgba(74, 131, 186, 0.18);
  background: rgba(255, 255, 255, 0.68);
}

.group-chat-window__toolbar {
  display: flex;
  gap: 4px;
  padding: 8px 12px;
...
.group-chat-window__input-wrapper {
  padding: 8px 12px;
}

.group-chat-window__input {
  width: 100%;
  height: 68px;
...
.group-chat-window__input::placeholder {
  color: rgba(45, 95, 143, 0.48);
}

.group-chat-window__actions {
  display: flex;
